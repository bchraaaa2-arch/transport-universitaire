...
export default function Bus() {
  const { bus, setBus, affecter, lignes } = useData();
  ...
  const toggle = (id) => setBus(bus.map(b => b.id === id ? { ...b, statut: b.statut === 'actif' ? 'maintenance' : 'actif' } : b));

  return (
    <Layout>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'Plus Jakarta Sans, sans-serif', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Flotte</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Total flotte',    value: bus.length,                                                     color: '#7c6ff7' },
          ...
          <div key={s.label} className="stat-card" style={{ '--accent-color': s.color, padding: '18px 20px' }}>
            <div style={{ fontSize: 24, fontWeight: 800, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>{s.value}</div>
          ...
          return (
            <div key={b.id} className="card" style={{ borderLeft: `3px solid ${isActif ? 'var(--green)' : 'var(--red)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                <div>
                  <div style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>{b.matricule_bus}</div>
              ...
              <div style={{ display: 'flex', gap: 20, marginBottom: 14 }}>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', fontFamily: 'Plus Jakarta Sans, sans-serif', letterSpacing: '0.04em' }}>Capacité</div>
                  <div style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, fontSize: 20, color: 'var(--text)' }}>{b.capacite_max}</div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>places</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', textTransform: 'uppercase', fontFamily: 'Plus Jakarta Sans, sans-serif', letterSpacing: '0.04em' }}>Ligne affectée</div>
                  {ligne
                    ? <div style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, fontSize: 14, color: 'var(--accent)', marginTop: 4 }}>{ligne.nom_ligne}</div>
              ...
              <div style={{ background: 'var(--surface2)', borderRadius: 8, padding: '8px 12px', marginBottom: 12 }}>
                <div style={{ fontSize: 10, fontFamily: 'Plus Jakarta Sans, sans-serif', color: 'var(--text-muted)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.04em' }}>Historique des affectations</div>
                {affecter.filter(a => a.id_bus === b.id).map((a, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-dim)', padding: '2px 0' }}>
                      ...
                      <span style={{ color: a.statut === 'actif' ? 'var(--green)' : 'var(--text-muted)', fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600 }}>
                        {a.date_debut} → {a.date_fin || "Aujourd'hui"}
                      </span>
                    </div>
                  );
                })}
                {affecter.filter(a => a.id_bus === b.id).length === 0 && <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Aucune affectation</div>}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn btn-ghost" style={{ flex: 1, fontSize: 12, justifyContent: 'center' }} onClick={() => { setForm({ ...b, capacite_max: String(b.capacite_max) }); setModal(b); }}>Éditer</button>
                <button className="btn" style={{ flex: 1, fontSize: 12, justifyContent: 'center', background: isActif ? 'rgba(239,68,68,.1)' : 'rgba(16,185,129,.1)', color: isActif ? 'var(--red)' : 'var(--green)', border: `1px solid ${isActif ? 'rgba(239,68,68,.2)' : 'rgba(16,185,129,.2)'}` }} onClick={() => toggle(b.id)}>
      ...
      {modal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal">
            <h2 style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontSize: 18, marginBottom: 20 }}>{modal === 'add' ? 'Ajouter un bus' : 'Modifier le bus'}</h2>
