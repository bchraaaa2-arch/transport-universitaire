...
export default function Trajets() {
  const { trajets, setTrajets, bus, horaires, lignes, incidents } = useData();
  ...
  return (
    <Layout>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'Plus Jakarta Sans, sans-serif', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Planning</div>
        <h1 style={{ fontSize: 26, fontWeight: 800 }}>Trajets</h1>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
        {[
          { label: 'Prévus',     value: trajets.filter(t => t.etat === 'prevu').length,     color: '#7c6ff7' },
      ...
      <div style={{ display: 'grid', gap: 12 }}>
        {filtered.map(t => {
          ...
          const incident = incidents.find(i => i.id_trajet === t.id);
          return (
            <div key={t.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr auto', gap: 0 }}>
                <div style={{ padding: '20px 24px', borderRight: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'Plus Jakarta Sans, sans-serif', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>Ligne</div>
                  <div style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: 'var(--text)', fontSize: 14 }}>{l?.nom_ligne || '—'}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{t.date_trajet}</div>
                </div>
                <div style={{ padding: '20px 24px', borderRight: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'Plus Jakarta Sans, sans-serif', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>Bus · Horaire</div>
                  <div style={{ fontFamily: 'monospace', fontSize: 13, color: 'var(--text)' }}>{b?.matricule_bus || '—'}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{h ? `${h.jour_semaine} · ${h.heure_depart}` : '—'}</div>
                </div>
                <div style={{ padding: '20px 24px', borderRight: '1px solid var(--border)' }}>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'Plus Jakarta Sans, sans-serif', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 4 }}>Remplissage</div>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                    <span style={{ fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: fillColor, fontSize: 20 }}>{pct}%</span>
                ...
                <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end', justifyContent: 'center' }}>
                  ...
                  {incident && <span className="badge badge-orange" style={{ fontSize: 10 }}>⚠ Incident</span>}
                  <select value={t.etat} onChange={e => updateEtat(t.id, e.target.value)} style={{ fontSize: 11, padding: '4px 8px', width: 'auto', marginTop: 4 }}>
        ...
        {filtered.length === 0 && <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>Aucun trajet dans cette catégorie</div>}
      </div>
    </Layout>
  );
}
